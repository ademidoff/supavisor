...
// waitingCalls are the os/exec helpers that wait for the child themselves.
// Every one of them races the reaper for the same status.
var waitingCalls = map[string]string{
	"CommandContext": "its context watcher is wired to Wait, and its Cancel hook can fire against a reaped and recycled PID",
...
func checkFileDoesNotWait(t *testing.T, path, rel string) {
	...
	ast.Inspect(file, func(n ast.Node) bool {
		...
		// cmd.Wait() specifically: bare Wait is also sync.WaitGroup's, which is
		// used correctly all over the daemon.
		if receiver, isIdent := selector.X.(*ast.Ident); isIdent && receiver.Name == "cmd" && selector.Sel.Name == "Wait" {
			t.Errorf("%s:%d calls cmd.Wait(). The reaper is the only waiter; take the status from the channel "+
				"spawn returned.", rel, fset.Position(call.Pos()).Line)
			return true
		}

		reason, forbidden := waitingCalls[selector.Sel.Name]
		if !forbidden {
			return true
		}
		// exec.Command itself is fine, and is how both allowed files start a child.
		if pkg, isIdent := selector.X.(*ast.Ident); isIdent && pkg.Name != "exec" && selector.Sel.Name == "CommandContext" {
			return true
		}
		t.Errorf("%s:%d calls %s, which must not be used here: %s. Start through "+
			"reaperFor(logger).spawn and read the status from the channel it returns.",
			rel, fset.Position(call.Pos()).Line, selector.Sel.Name, reason)
		return true
	})
}
